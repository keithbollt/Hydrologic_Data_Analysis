...
>Each row from the metadata represents a different type of data being collected.
Look at the vignette for information on what the metadata stands for.

What is the period of record for mean daily discharge at each of these sites?

> Eno River (actually the Haw River): 
...
Statcd 00003, mean discharge, is the default.
parametercd of 00060 is discharge
...
>On average, the magnitude varies greatly. While all the rivers have peaks that are on a similar scale, the typical quotidien discharge on, say, the Verde River is much lower than, say, the Bitterroot River.
...
>The Bitterroot, Sauk, Nehalem all have seasonal cycles. These are the three rivers that are in regions with exageratted seasons.
...
> You did faceting last semester, you would go back and use that code.
Use the function "rbind" to stack the four dataframes on top of eachother (assuming site number has the same title in each dataframe, which it does), then facet by site number.
...
BitterrootDischarge.Historical <- mutate(BitterrootDischarge.Historical, 
                                         DOY = yday(Date))  # Adds a DOY column
