...
options(scipen = 100)  #avoids scientific notation up to e^100
...
<Lagos has a github page that is useful. See link below>
...
LAGOStrophic <- 
  ...
  drop_na(chla:secchi)  # If there is an na in any cell in these three columns, remove the entire row
...
ggplot(LAGOStrophic, 
  ...
  geom_jitter(alpha = 0.2) +     #geom_jitter spreads out data within a month
...
> Secci depth appears to be positively correlated with chlorophyll level from the way the ggplots are made. In reality, they are inversely correlated.
         ...
         trophic.class = 
            ifelse(TSI.chl < 40, "Oligotrophic", 
                   ifelse(TSI.chl < 50, "Mesotrophic",
                          ifelse(TSI.chl < 70, "Eutrophic", "Hypereutrophic")))) #Adds a column that labels the indices by range. The different indices don't all agree with eachother on what label to assign.
...
```{r}
write.csv(LAGOStrophic, "./Data/LAGOStrophic.csv")
```
