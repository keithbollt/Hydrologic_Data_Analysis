...
Putting a $ at the beginning and end of an equation makes it an equation according to R.
...
attr(EnoDischarge, "siteInfo")  # Look up the site info
...
EnoRecurrence <-    #Creates recurrence intervals for 1963-1992
  EnoDischarge %>%
  filter(Year < 1993) %>%
  group_by(Year) %>%
...
Eno.RImodel <- lm(data = EnoRecurrence, PeakDischarge ~ log(RecurrenceInterval))  # Let's model it
summary(Eno.RImodel)  

#What is the discharge for a 100-year flood in this system? a 500-year flood?
Eno.RImodel$coefficients[1] + Eno.RImodel$coefficients[2]*log(100)  # You can call up any model's coefficient
...
>A given discharge has a smaller recurrence interval if we use the complete dataset. You would plan for bigger, more frequent floods if you use the complete dataset. This means more robust, more expensive infrastructure. That said, should we even be using this simple of a data analysis if past performance does not predict future results.
...
Salado.RImodel <- lm(data = SaladoRecurrence, PeakDischarge ~ log(RecurrenceInterval))  # Let's model it
summary(Salado.RImodel)  

#What is the discharge for a 100-year flood in this system? a 500-year flood? 
Salado.RImodel$coefficients[1] + Salado.RImodel$coefficients[2]*log(100)  # You can call up any model's coefficient
...
#We already added a year column to the Salado discharge dataset in the previous analysis
...
Salado.RImodel.full <- lm(data = SaladoRecurrence.full, PeakDischarge ~ log(RecurrenceInterval))  # Let's model it
summary(Salado.RImodel.full) 

#What is the discharge for a 100-year flood in this system? a 500-year flood? 
Salado.RImodel.full$coefficients[1] + Salado.RImodel.full$coefficients[2]*log(100)  # You can call up any model's coefficient
Salado.RImodel.full$coefficients[1] + Salado.RImodel.full$coefficients[2]*log(500)
```
