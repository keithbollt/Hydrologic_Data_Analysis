...
> Crane Creek, it's flatter as flow increases
...
Diluting: Concentration decreases as flow increases
Flushing: Concentration increases as flow increases
Chemostatic: Concentration not related to flow

A hysteresis loop occurs when a stream's concentration loops back on itself as the river rises and then falls.
...
>Measures the rate that an ecosystem consumes energy

Why would we want to know the *GPP* and *ER* of a river reach?

> They tell us about that stream's ecology
...
```{r}
Enodat <- request_data(   #streamPULSE package
  sitecode = "NC_Eno",
  variables = c('DO_mgL'),
...
> Pt is productivity/photosynthesis
Rt is respiration
Dt is water discharge
