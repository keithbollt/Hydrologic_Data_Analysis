...
print(Physicalprofiles2016)    # This is how you put three graphs that can't be faceted bc they have different variables on the same screen.

```

How do temperature profiles vary over the course of a season? What are the mechanisms for this pattern?

> We see the temperature gradient shifting warmer as the summer goes on. The hypolimnion remains in the same location.

How do irradiance profiles vary over the course of a season? What are the mechanisms for this pattern?

> Irridescence increases until the summer solstice, then decreases thereafter.It also decreases with depth

How do dissolved oxygen profiles vary over the course of a season? What are the mechanisms for this pattern?

> Dissolved oxygen capacity is inversely correlated with temperature, so it makes sense that we see dissolved oxygen and temperature moving in opposite directions, where there is change (there isn't much change at the bottom of the lake).The spike in oxygen at mid depth is caused by a spike in cholorophyll. There is almost no oxygen at depth due to microbial respiration, and it is too deep for it to be light enough for photosynthesis to occur.
...
Pauldatafiltered <- Pauldata %>%
  filter(depth == 0 & Month < 11 & Month > 4) 

Pauldata.May <- Pauldatafiltered %>%
  filter(Month == 5)

PaulMaylineartest <- lm(data = Pauldata.May, temperature_C ~ year4)
summary(PaulMaylineartest)

# You do linear tests with lm
