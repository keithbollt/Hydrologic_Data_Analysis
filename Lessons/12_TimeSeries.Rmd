...
Mendotadata <- Mendotadata %>%
  ...
  arrange(sampledate)  # this lines up the dates in order
...
* **Piecewise constant**: <have a point equal to the known value it is nearest>
* **Linear**: <connect the dots>
* **Spline**: <draws a curved line connecting the points>
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear")) # there are 303 months in the dataset
linearinterpolation$x <- as.Date(linearinterpolation$x, origin = "1970-01-01") #the data spat out days since 1970, so we'll change it to dates
...
Mendotatimeseries <- ts(linearinterpolation$TP, frequency = 12, 
                        start = c(1988, 7, 5), end = c(2013, 8, 5))  #starts on 7/5/1988, ends on 8/5/2013
# Run SMK test
...
#S counts the difference between the points. A larger absolute value of varS means more variation in the data.
#No matter when you start the dataset, season 1 is always January
```

What would we conclude based on these findings? Describe as you would to an educated but non-specialist audience.

> There is not a significant overall trend, or a trend in 10 of 12 months, but there is a significant negative trend in October and November.
...
ClearCreekDischarge.Monthly <- ClearCreekDischarge %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>%   # You want an average for every month of every year
  summarise(Discharge = mean(Discharge))
    
ClearCreek_ts <- ts(ClearCreekDischarge.Monthly[[3]], frequency = 12)  #month is column 3; 12 months in a year
